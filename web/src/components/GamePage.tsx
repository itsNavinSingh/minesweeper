...
function GamePage() {
  ...
  const increaseLevel: () => void = () => {
    const newlevel = currentlevel.number % 10;
    setCurrentlevel(levels[newlevel]);
  }
  useEffect(()=> {
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <header className="h-16 bg-blue-500 text-white flex items-center justify-center p-4">
        ...
        <select
          ...
          className="p-2 rounded text-gray-800 text-xl font-semibold bg-blue-300"
      ...
      <main className="flex-1 flex items-center justify-center  p-4">
        {wasmReady ? (
          <Minesweeper boardSize={currentlevel.size} mineCount={currentlevel.mine} nextLevel={increaseLevel} />
        ) : (
